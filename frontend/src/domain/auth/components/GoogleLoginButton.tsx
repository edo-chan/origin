import React, { useCallback } from 'react';
import { useAuth } from '../hooks';
import * as styles from './GoogleLoginButton.css';

interface GoogleLoginButtonProps {
  /** Text to display on the button */
  text?: string;
  /** Redirect URI after successful authentication */
  redirectUri?: string;
  /** Button variant */
  variant?: 'default' | 'compact' | 'full-width';
  /** Whether to show error messages */
  showErrors?: boolean;
  /** Custom CSS class */
  className?: string;
  /** Callback fired when login starts */
  onLoginStart?: () => void;
  /** Callback fired when login succeeds */
  onLoginSuccess?: () => void;
  /** Callback fired when login fails */
  onLoginError?: (error: string) => void;
  /** Additional props to pass to the button element */
  buttonProps?: React.ButtonHTMLAttributes<HTMLButtonElement>;
}

/**
 * Google OAuth login button component
 * Handles the complete OAuth flow with loading states and error handling
 */
export function GoogleLoginButton({
  text = 'Sign in with Google',
  redirectUri,
  variant = 'default',
  showErrors = true,
  className,
  onLoginStart,
  onLoginSuccess, 
  onLoginError,
  buttonProps = {}
}: GoogleLoginButtonProps) {
  const { login, isLoading, error, clearError } = useAuth();

  const handleLogin = useCallback(async () => {
    try {
      clearError();
      onLoginStart?.();
      
      console.info('User Action', {
        action: 'google_login_button_clicked', 
        component: 'GoogleLoginButton',
        redirectUri,
        timestamp: new Date().toISOString()
      });

      await login(redirectUri);
      onLoginSuccess?.();
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : 'Login failed';
      onLoginError?.(errorMessage);
      
      console.error('User Action', {
        action: 'google_login_failed',
        component: 'GoogleLoginButton', 
        error: errorMessage,
        timestamp: new Date().toISOString()
      });
    }
  }, [login, redirectUri, onLoginStart, onLoginSuccess, onLoginError, clearError]);

  const getButtonClasses = () => {
    let classes = styles.googleLoginButton;
    
    if (variant === 'compact') {
      classes += ` ${styles.compactButton}`;
    } else if (variant === 'full-width') {
      classes += ` ${styles.fullWidthButton}`;
    }
    
    if (className) {
      classes += ` ${className}`;
    }
    
    return classes;
  };

  return (
    <div>
      <button
        type="button"
        className={getButtonClasses()}
        onClick={handleLogin}
        disabled={isLoading}
        aria-label={isLoading ? 'Signing in...' : text}
        {...buttonProps}
      >
        {isLoading ? (
          <>
            <div className={styles.loadingSpinner} aria-hidden="true" />
            <span className={styles.buttonText}>Signing in...</span>
          </>
        ) : (
          <>
            <GoogleIcon />
            <span className={styles.buttonText}>{text}</span>
          </>
        )}
      </button>
      
      {showErrors && error && (
        <div 
          className={styles.errorMessage}
          role="alert"
          aria-live="polite"
        >
          {error.message}
        </div>
      )}
    </div>
  );
}

/**
 * Google logo SVG icon component
 */
function GoogleIcon() {
  return (
    <svg
      className={styles.googleIcon}
      viewBox="0 0 24 24"
      aria-hidden="true"
    >
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}