...
export const GoogleLoginButton: React.FC<GoogleLoginButtonProps> = ({
  ...
  const handleClick = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    try {
      ...
      // This would typically use the Google Identity Services library
      const mockUser = {
        id: 'mock-user-id',
  ...
  return (
    <button
      ...
      aria-label={`${text} - Sign in with your Google account`}
      type="button"
      {...props}
    >
      {!isLoading && <GoogleLogo className={googleLogoIcon} />}
      <span className={buttonText}>{text}</span>
      
      {isLoading && (
        <div className={loadingOverlay}>
          <div className={buttonSpinner} aria-label="Loading..." />
        </div>
      )}
    </button>
  );
