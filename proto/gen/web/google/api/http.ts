 ...
 * HTTP | gRPC
 * -----|-----
 * `GET /v1/messages/123456?revision=2&sub.subfield=foo` | `GetMessage(name: "messages/123456" revision: 2 sub: SubMessage{subfield: "foo"})`
 *
 ...
 * specifies the mapping. Consider a REST API that supports creating and updating a
 ...
 *       string parent = 1; // Mapped to URL query parameter `parent`.
 *       Message message = 2;   // Mapped to body.
 *     }
 *     message UpdateMessageRequest {
 *       Message message = 1;   // Mapped to body.
 *     }
 *
 ...
 * -----|-----
 * `GET /v1/messages/123456` | `GetMessage(name: "messages/123456")`
 * `POST /v1/messages { "text": "Hi!" }` | `CreateMessage(parent: "messages", message { text: "Hi!" })`
 * `PATCH /v1/messages/123456 { "text": "Hi!" }` | `UpdateMessage(message { name: "messages/123456", text: "Hi!" })`
 *
 ...
 *       rpc UpdateMessage(UpdateMessageRequest) returns (Message) {
 *         option (google.api.http) = {
 *             patch: "/v1/{name=messages/*}"
 *             body: "*"
 *         };
 *       }
 *     }
 *     message UpdateMessageRequest {
 *       string name = 1; // mapped to the URL
 ...
 * HTTP | gRPC
 * -----|-----
 * `PATCH /v1/messages/123456 { "text": "Hi!" }` | `UpdateMessage(name: "messages/123456" message { text: "Hi!" })`
 *
 * Note that when using `*` in the body mapping, it is not possible to
 * have HTTP parameters, as all fields not bound by the path end in
 * the body. This makes this option more rarely used in practice when
 * defining REST APIs. The common usage of `*` is in custom methods
 * which don't use the URL at all.
 *
 ...
 *     message GetMessageRequest {
 *       string name = 1; // Mapped to URL path.
 *       string user_id = 2; // Mapped to URL path.
 *       string message_id = 3; // Mapped to URL path.
 *     }
 *
 * This enables the following two alternative HTTP JSON to RPC mappings:
 *
 ...
 * have control over the proto source.
 *
 * Example:
 *
 *     http:
